        ...
        for f in range(len(freqs_uni)):  # Loop through all different frequencies
            ...
            for a in fifield_volt[freqs_uni[f]].keys():  # Only loop through amps that exist
                ...
                for trial in range(repeats):
                    ...
                    spike_count[trial] = len(
                        detect_peaks(x, mph=peak_params['mph'], mpd=peak_params['mpd'], threshold=0, edge='rising', kpsh=False, valley=peak_params['valley'],
                                        show=peak_params['show'], ax=None))
...
def get_soundfilestimuli_data(datasets, tag):
    ...
    voltage = mtag.retrieve_data(0, 0)[:]  # Voltage Trace
    voltage_time = np.linspace(0, len(voltage) / sampling_rate, len(voltage)) * 1000  # in ms

    # Plot sound stimulus and voltage trace
    plt.subplot(2, 1, 1)
    plt.plot(voltage_time, voltage, 'k')
    plt.title('Stimulus: %s' % sound_file_name)

    plt.subplot(2, 1, 2)
    plt.plot(sound_time, sound[1], 'k')
    return 0
    ...
    for dat in range(len(datasets)):
        ...
        for i in voltage:
            ...
            for trial in range(voltage[i]['trials']):
                x = voltage[i][trial][0]
                if trial == show_trial_detection and (i == 1 or i == 4):
                    spike_times = detect_peaks(x, mph=peak_params['mph'], mpd=peak_params['mpd'], threshold=0,
                                               edge='rising', kpsh=False,
                                               valley=peak_params['valley'], show=True, ax=None)
                else:
                    spike_times = detect_peaks(x, mph=peak_params['mph'], mpd=peak_params['mpd'], threshold=0, edge='rising', kpsh=False,
                             valley=peak_params['valley'], show=peak_params['show'], ax=None)
...
def rect_intervals_spike_detection(datasets, peak_params):
    for dat in range(len(datasets)):  # Loop through all recording sessions in the list
        data_name = datasets[dat]
        pathname = "figs/" + data_name + "/"
        ...
        for i in voltage:
            x = voltage[i]['voltage']
            spike_times = detect_peaks(x, mph=peak_params['mph'], mpd=peak_params['mpd'], threshold=0, edge='rising',
                                       kpsh=False,
                                       valley=peak_params['valley'], show=peak_params['show'], ax=None)

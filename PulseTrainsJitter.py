...
def get_ratios(max_rand, duration):
    # Load call series
    if stim_type is 'series':
        calls = sio.loadmat('/media/nils/Data/Moth/CallStats/CallSeries_Stats/samples.mat')['samples'][0]
        ...
        for z in tqdm(range(len(rands)), desc='Jitters'):
            ...
            if plot_ratios:
                ...
                figname = '/media/nils/Data/Moth/CallStats/pulsetrains/figs/Distance_Ratios_ISI_SYNC_' +\
                          stim_type + '_' + str(z) + '_' + str(int(max_rand*1000)) + '.pdf'
                fig.savefig(figname)
                plt.close(fig)
                # print('ISI and SYNC plots saved for: ' + str(max_rand*1000) + ' ms')
                ...
                figname = '/media/nils/Data/Moth/CallStats/pulsetrains/figs/Distance_Ratios_DUR_COUNT_' + stim_type + \
                          '_' + str(z) + '_' + str(int(max_rand*1000)) + '.pdf'
                fig.savefig(figname)
                plt.close(fig)
...
if compare_ratios:
        ...
        # c1.set_ticks([0, 0.5, 1, 1.5, 2])
        # c1.set_ticklabels([0, 0.5, 1, 1.5, 2])
        ...
        ax7.set_yticks(np.arange(0, 101, 20))
        ax8.set_yticks(np.arange(0, 101, 20))

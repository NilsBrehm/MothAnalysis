...
def plot_volt(rec, name, stim_type, volt, spikes, valleys, spikes_old, cutoff, trial_nr, t_limit, ID, path_names):
    ...
    # Filter and remove large spikes
    marked = [[]] * len(call_volt)
    y = [[]] * len(call_volt)
    for i in range(len(call_volt)):
    ...
    grid = matplotlib.gridspec.GridSpec(nrows=trial_count+4, ncols=1)
    ...
    for k in range(len(call_spikes)):
        ...
        ax3.plot(call_vals[k]*1000, np.ones(len(call_vals[k])) + k, 'bs', markersize=0.2, linewidth=0.5)
        ax4.plot(call_old[k]*1000, np.ones(len(call_old[k])) + k, 'gs', markersize=0.2, linewidth=0.5)

    for j in range(trial_count):
        ax = plt.subplot(grid[j+4, 0])
        ...
        ax.plot(call_spikes[j]*1000, y[j][marked_idx.astype('int')], 'ro', linewidth=1, markersize=1)
        ax.plot(call_vals[j]*1000, y[j][vals_idx.astype('int')], 'bo', linewidth=1, markersize=0.7)
        ax.plot(call_old[j]*1000, y[j][old_idx.astype('int')], 'gx', linewidth=1, markersize=0.5)
...
#                      window=window, mph_percent=mph_percent)
...
for q in range(len(ss)):  # Loop through stimulus types
    ...
    for kk in tqdm(range(len(stims)), desc=stim_types):
        plot_volt(rec, stims[kk], stim_types, volt, spikes, valleys, spikes_old, cutoff, None, [0, call_limit], ID=kk, path_names=path_names)
